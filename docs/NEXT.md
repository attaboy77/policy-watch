...
마지막 갱신: 2026-09-10, 메일 발송 이력(구글 뉴스 재등장 중복 발송) + 뉴스 필터 2종 + 메일 언론 발행일 필터 추가 세션 직후 기준. **Phase 6(GitHub Pages 자동 배포) 완료·운영 중.**
...
"2026-09-09 세션 요약" 참고. 테스트 571개 통과. **미검증(다음 Actions 실행 필요)** —
아래 "2026-09-10 세션 요약"과 그 "후속" 절의 "미검증" 항목 참고.
...
### 후속: 메일 언론(L3) 발행일 필터 추가(어제·오늘만, 공식은 면제)

같은 날 사용자가 이어서 요청 — 구글 뉴스가 오래된 기사를 뒤늦게 결과에
포함시키는 경우가 있는데(위 발송 이력 기능은 "한 번 보낸 걸 또 보내는 것"만
막지, "처음 보내는데 사실 오래된 기사인 것"은 못 막는다), 그런 기사가 메일에
나가는 걸 원치 않는다는 요청.

- **`notify_mail.is_recent_enough_for_mail(published_at, today=None)` 신규**
  — 발행일이 오늘 또는 어제(KST)면 True. `MAIL_NEWS_RECENCY_DAYS = 1`로
  범위를 상수화(0=오늘만, 1=어제·오늘). `_run()`에서 `new_news`(L3)에만
  적용 — `new_official`(L1/L2)은 필터를 안 거친다(사용자 지시: "공식 문서는
  발표일이 지나도 챙겨야 하니까").
  - **사이트(site/data.json, 대시보드)는 안 건드린다** — 이 필터는
    `notify_mail.py`(메일 발송 직전 단계)에만 있고, `main.py`의
    `build_data_json()` 파이프라인에는 없다. 필터링된 기사는 여전히 다음
    실행의 PREV_DATA_JSON 백업(=오늘자 site/data.json)에 포함되므로, 내일부터는
    자동으로 "신규"가 아니게 된다 — 별도로 `_sent_log`에 기록할 필요 없음
    (실제로 메일에 실린 것만 기록해야 하므로 기록하지도 않는다).
  - **발행일을 못 구하는 항목 처리(사용자 질문에 대한 답)**: 포함시킨다
    (보수적으로 True). 다만 실무적으로 이 경우는 사실상 발생하지 않는다 —
    `_utils.finalize_item()`이 이미 `published_at`이 없으면 `collected_at`
    (오늘 수집 시각) 날짜로 채워두기 때문에(SPEC §4 필드 규칙, 기존 동작),
    스키마상 `published_at`은 항상 "YYYY-MM-DD" 문자열이다. 그래도 방어적으로
    파싱 실패(형식 깨짐 등 예상 밖 상황)는 "빠뜨리는 것보다 보여주는 게
    낫다"는 원칙으로 포함 처리.
  - 테스트: `TestIsRecentEnoughForMail` 6개(오늘/어제/2일 전/과거/결측/형식
    오류) + `_run()` 종단 3개(공식 면제 확인, 언론만 오래됐을 때 발송 자체
    스킵, 통과 시 정상 발송) + 발송 이력에 필터링된 항목이 안 남는지 1개.
    기존 `test_sends_when_new_news_only_item_exists`가 고정된 과거 날짜를
    쓰고 있어 이번 필터에 걸려 깨졌던 것도 오늘 날짜로 맞춰 수정.
- 테스트 560→571개 통과.
- **미검증**: 다음 실제 발송에서 (a) 어제·오늘이 아닌 구글 뉴스 기사가 정말
  메일에서 빠지는지, (b) 그런데도 대시보드엔 그대로 나오는지, (c) 공식 자료는
  날짜와 무관하게 계속 발송되는지 확인 필요.
