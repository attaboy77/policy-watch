...
사용자 보고: 9/9 메일에 "공식 기관 발표 [세법] 10건"(국세청 개정세법 해설
2017~2026년치)이 9/4에 이어 재발. 사용자 가설은 "프록시 경유 시 id가 URL 기반이라
바뀌었을 것" — 진단해보니 id 자체는 문제가 아니었고, 더 근본적인 버그를 발견·수정했다.
같은 세션에서 law_api "60초 초과" 3일 연속 실패 대응(상세 로깅 + 임시 상한)도 함께
진행했고, 마지막엔 원인 불명 소스 3개를 코드 수정 없이 끄고 켤 수 있는 설정으로 뺐다.

- **진단(구현 전 먼저 확인) — id는 프록시와 무관, 진짜 원인은 "빈 결과=성공" 버그**:
  `nts.py`/`moef.py`/`fsc.py`/`policy_briefing.py` 전부 id를 각 파일에 하드코딩된
  원본 도메인 URL로 조립해서 프록시 경유 여부와 무관함을 코드로 확인. 대신
  `data/source_cache.json` git 히스토리를 추적해 실측: 프록시 붙이기 직전
  캐시엔 그 10건(id까지 사용자 보고와 일치)이 있었는데, 프록시 붙인 직후 첫
  Actions 실행에서 nts/moef/fsc/policy_briefing 4개 전부 캐시가 0건으로
  바뀜 — 그런데 `meta.sources_ok`엔 4개 다 "성공"으로 찍혀 있었다. 원인:
  `main.py`의 `collect_all()`이 공식 소스는 "예외 없이 반환"만 성공으로 보고
  빈 리스트도 그대로 캐시에 덮어씀(NEWS_SOURCES는 `if normalized:` 가드가
  이미 있었는데 OFFICIAL_SOURCES엔 빠져 있었음). 그래서 프록시 전환 직후
  0건이 성공으로 기록되며 멀쩡하던 전일 캐시를 지웠고, 다음에 nts가 진짜
  성공하니 기준선이 0건이라 10건 전부 신규로 오판됐다.
- **수정 1 — nts 연도 필터**: `nts.py`에 `_filter_recent_years()` 추가 —
  제목이 "YYYY년 ..."로 시작하는 항목은 목록에 실제 있는 연도 중 최근
  2개년치만 남긴다(오늘 날짜가 아니라 데이터에 있는 연도 기준이라 발행
  공백기에도 0건이 안 됨). 이 게시판이 애초에 10년치를 매번 수집 대상에
  넣을 이유가 없다는 사용자 판단.
- **수정 2 — OFFICIAL_SOURCES에도 빈 결과=실패 가드 추가**: 위에서 찾은
  진짜 원인을 직접 수정 — `collect_all()`의 OFFICIAL_SOURCES 루프에
  `if got:` 가드를 추가해, 예외 없이 빈 리스트가 와도 "결과 0건(응답 없음
  또는 파싱 실패)"로 실패 처리하고 전일 캐시로 폴백(캐시도 안 덮어씀).
  NEWS_SOURCES와 동일 패턴으로 통일.
- **law_api Actions 상세 로깅 + 임시 120초 상한**: 사용자 보고 — 로컬 실측
  13.5초/요청 14회인데 Actions는 이틀 연속(당시 기준) "60초 초과". `law_api.py`에
  `_timed_get_govt()` 신설 — 요청 번호·소요시간(재시도 포함)·HTTP 상태를
  요청마다 로그, `fetch()` 끝에 총 요청 수/총 소요시간 요약 출력. `main.py`의
  `_fetch_with_timeout()`이 소스별 timeout을 인자로 받도록 바꾸고
  `SOURCE_TIMEOUT_OVERRIDES = {"law_api": 120}` 추가(다른 소스는 60초 유지) —
  원인 파악 전 임시 응급조치. **다음 Actions 실행에서 `[law_api] 요청 #N ...`
  줄들로 원인(프록시 왕복 지연/law_api만 느림/재시도 중첩) 확인 필요.**
- **소스 비활성화 설정 신설**: moef/fsc/policy_briefing 3개는 위 수정 이후에도
  프록시 경유 시 0건이 계속 나왔고(캐시도 없어 폴백할 데도 없음), 원인이
  아직 안 밝혀진 채로 로그·메일에 실패 경고만 쌓여 사용자가 일단 끄기로
  결정. `data/tax_subjects.yml`과 같은 패턴(YAML + `enabled` 플래그)으로
  `data/source_toggles.yml` 신설, `_config._load_disabled_sources()`가
  읽어 `DISABLED_SOURCES`로 노출. `main.py`의 `collect_all()`이 두 루프
  (OFFICIAL_SOURCES/NEWS_SOURCES) 시작에서 `if name in DISABLED_SOURCES:
  continue`로 아예 시도하지 않는다 — `sources_ok`/`sources_failed` 어디에도
  안 잡히고, `record_failure`/`record_success` 둘 다 호출 안 돼 연속 실패
  카운트도 안 늘어난다. 코드 수정 없이 `enabled: true`로 되돌리면 다시 켜짐.
  세 소스 다 `reason:` 필드에 "프록시 경유 시 0건 반환, 원인 미규명
  (2026-09-09)"을 남겨둠 — **다음 조사 때 확인할 것: 응답 자체가 없는지
  (connect/read timeout) 아니면 응답은 오는데 파싱이 안 되는지(예: WAF가
  200으로 다른 페이지를 돌려줌) 구분 필요.**
- 테스트 513→534개(신규: `test_nts.py` 6개, `test_config.py` 7개,
  `test_main.py`에 8개 추가 — 타임아웃 오버라이드 2개, 빈 결과 폴백 2개,
  비활성화 소스 4개).
- **미검증**: 위 전부 다음 Actions 실행으로 확인 필요 — (a) nts가 최근
  2개년치만 반환하는지, (b) law_api가 120초 안에 끝나고 상세 로그가
  찍히는지, (c) moef/fsc/policy_briefing이 `sources_ok`/`sources_failed`
  어디에도 안 잡히고 로그에 "수집 시도" 자체가 안 뜨는지, (d) 메일에
  이 3개에 대한 "전일 데이터 사용" 경고가 더 이상 안 뜨는지.
...
사용자가 메일/대시보드에서 구글 뉴스로 수집된 기사 링크가 전부 구글의 자동화 요청
차단 페이지로 연결된다고 보고(네이버 뉴스는 정상). 원인·해결책을 실측으로 조사한 뒤
사용자 승인 받아 구현·검증까지 완료.

- **원인 조사(실측)**: RSS `<link>`/`<description>`/`<source url>` 전부 실제 원문 URL을
  안 담고 있음(뉴스 리다이렉트 자체 또는 매체 홈페이지 도메인뿐). 단순 302 추적도
  안 됨(구글이 JS로 클라이언트 사이드 리다이렉트 처리). `CBMi...` base64는 opaque
  토큰이라 직접 디코딩 불가(예전엔 평문 URL이었으나 구글이 포맷을 바꿈). 대신 그
  링크를 GET하면 응답 HTML에 `data-n-a-id`/`data-n-a-sg`/`data-n-a-ts` 3개 값이 있고,
  이를 구글 내부 `https://news.google.com/_/DotsSplashUi/data/batchexecute` 엔드포인트에
  POST하면 실제 원문 URL을 돌려준다는 걸 확인(로컬 IP 6/6 성공, 제목·매체명 전부 실제
  기사와 일치 확인). **비공식 내부 엔드포인트**라 예고 없이 바뀔 수 있음(base64
  직접 디코딩 방식에서 이미 한 번 이 방식으로 바뀐 이력이 있음) — 그래서 구현은
  실패 시 예외를 삼키고 원래 구글 링크로 반드시 폴백하도록 설계.
- **구현**: `sources/_google_decode.py` 신규(저수준 디코딩 함수, 실패 시 None).
  `sources/google_news.py`에 `resolve_finalized_urls(items)` 신규 — **main.py 필터
  파이프라인 맨 끝(finalize 직후)에서만** 호출한다. 처음엔 `fetch_category()` 수집
  시점에 바로 디코딩했으나, 실측해보니 노이즈 필터만 거친 원본이 294건(kifrs91/
  tax5/icfr98/esg100)인데 최종 생존은 38건뿐이라 — 파이프라인 초입에서 디코딩하면
  나중에 걸러질 256건까지 구글에 불필요한 요청을 보내 시간·차단 위험만 키움을
  확인하고 즉시 끝단으로 옮김(사용자 지시: "구글이 속도로 차단할 수 있으니 간격을
  둬라"에 대한 직접 대응). id는 원래 구글 링크 기준 그대로 유지(디코딩 성공 여부로
  id가 실행마다 바뀌면 dedupe/요약 캐시/신규 항목 메일 판정이 전부 깨지므로) —
  `urls.news` 필드만 더 나은 값으로 교체.
  - 요청 간 1초 간격(`DECODE_SLEEP_SECONDS`), 항목당 요청 2회(GET+POST).
  - 성공/시도 건수를 크롤링 로그에 남김: `[google_news] URL 복원 X/Y건, 실패 Z건`
    (실측 크롤링 로그: `39/39건, 실패 0건`).
  - `meta.google_decode_stats`(`{attempted, success}`)로 `site/data.json`에도 기록
    (`_schema.py`에 옵션 필드 추가).
  - `notify_mail.py`에 `decode_notice_lines()` 신규 — 실패율 30% 초과(최소 시도
    5건 이상일 때만) 시 메일 하단에 경고 문구 추가(사용자 지시: "실패율이 높아지면
    알 수 있게").
  - 네이버 뉴스는 API가 애초에 `originallink`(실제 언론사 URL)를 직접 줘서 이 문제
    자체가 없음(naver_news.py 확인) — 다만 `NAVER_CLIENT_ID/SECRET` 미설정으로 현재
    실제 수집 중인 네이버 항목은 0건.
- **소급 백필**: 기존 `site/data.json`의 구글 뉴스 링크 38건을 전부 디코딩해
  `urls.news`만 교체(id·title 등 다른 필드는 절대 안 건드림 — git diff로 확인,
  `urls.news` 38쌍 외 변경 0줄). 38/38 성공, 두 차례 실측(1m50s, 1m34s) — 항목당
  평균 2.5~2.9초(요청 2회 + 1초 sleep).
- **속도·시간 실측**: 최종 생존 항목만 디코딩하는 현재 설계 기준 38~39건에
  1분30초~1분50초 소요(항목당 ~2.5~2.9초). 만약 파이프라인 초입(원본 294건)에서
  했다면 약 8배(294건 × ~2.9초 ≈ 14분+)까지 늘었을 것 — 최종 설계가 그 낭비를 피함.
- **실전 검증(로컬)**: `python -m sources.main` 전체 실행 1회 — `[google_news] URL
  복원 39/39건, 실패 0건` 확인, 스키마 검증 통과, 재확인 후 이 실행 결과 자체는
  커밋하지 않음(로컬 DNS 문제로 `law.go.kr` 전체 실패 — 세법 항목이 7건으로 급감,
  이 환경 특유의 문제라 실제 배포 데이터로 부적절 판단, `site/data.json`은 원래
  HEAD로 되돌리고 38건 백필만 재적용).
- 테스트 441→496개 통과(`sources/_google_decode.py` 8개, `google_news.py` 디코딩
  관련 9개, `notify_mail.py` `decode_notice_lines` 5개 등).
- **미검증 — 다음 세션 최우선**: 이번 테스트는 전부 **로컬 IP**에서 성공했다.
  GitHub Actions(공유 IP)에서도 같은 방식이 통하는지는 아직 실측 안 됨 — push 후
  `workflow_dispatch`(force_mail 무관, crawl 잡이 도는 트리거면 됨)로 수동 실행해
  Actions 로그에서 `[google_news] URL 복원 ...` 줄을 확인해야 한다. **gh CLI/GitHub
  API 토큰이 이 환경에 없어 워크플로 수동 트리거를 에이전트가 직접 못 함** —
  사용자가 GitHub Actions 탭에서 "Run workflow"로 직접 실행하고 로그를 공유해야
  다음 단계(로컬 전용 성공인지, 실제로 안정적인지 판단) 진행 가능.

### 후속: 첫 수동 실행이 12분+ 멈춰서 취소됨 → 소스 타임아웃 부재 발견·수정 (`c2ff1ca`)

위 검증을 위해 사용자가 처음 `workflow_dispatch`를 돌렸는데, `crawl` 잡이 12분
넘게 로그 진전 없이 멈춰(마지막 줄이 `[tax] 활성 세목 6개: ...` — 이건 import-time
로그로 첫 소스 시작 전) 사용자가 취소. **디코딩 단계까지 못 가서 이번 세션 작업과는
무관**, 별개의 사전 존재하던 구조적 문제로 확인:

- 개별 HTTP 요청(`_http.py get()`)엔 `timeout=15`가 이미 connect+read 양쪽에
  적용되고 있었음(재시도 3회+백오프까지 최악 ~46.5초/요청) — 문제는 **소스
  하나(fetch 함수 전체)에 합산 상한이 없었다**는 것. kasb.py `fetch()`는 하위
  fetch 5개를 순차 호출하며 그 안에서 상세 페이지를 여러 번 요청 — 요청마다는
  타임아웃이 걸려도 합쳐지면 사실상 무한정에 가깝게 밀릴 수 있었음. 첫 소스인
  kasb.or.kr이 GitHub Actions IP에서 law.go.kr처럼 느리거나 막혔을 가능성이
  높음(PROXY_BASE는 law_api.py에만 연결돼 있어 kasb는 우회 경로 없음 — 다음
  실측에서 실제로 kasb에서 멈추는지 확인 필요).
- `sources/main.py`에 `_fetch_with_timeout()` 신설 — 소스 fetch 함수를 별도
  스레드에서 실행, `SOURCE_TIMEOUT_SECONDS`(60초) 안에 안 끝나면 실패로 간주해
  기존 전일 캐시 폴백 경로로 넘어감. 소스 시작 직전 `[main] 수집 시도: {라벨}
  ({name})` 로그 추가 — 다음에 막히면 정확히 어느 소스인지 바로 보임.
- 테스트 8→13개(test_main.py), 전체 496→501개 통과. **미검증**: 이것도 로컬에서만
  확인(모킹된 느린 함수로 60초 안 기다리고 폴백되는지, 실행시간 측정으로 뒤 소스가
  안 밀리는지). GitHub Actions 재실행으로 (a) 어느 소스가 실제로 느린지 로그로
...
### 후속: 프록시 4개는 살아남, law_api만 이틀 연속 60초 초과 — 요청량 문제로 확인·수정

`PROXY_BASE` 값 재등록 + moef/nts/fsc/policy_briefing 전환 커밋 후 재실행 결과:
성공 8개, 실행시간 12분→4분45초, "PROXY_BASE 미설정" 경고도 안 뜸 — 4개는 완전히
해결. 그런데 **law_api만 여전히 "60초 초과"로 실패**(연속 2일째) — 유일하게
원래부터 프록시가 연결돼 있던 소스인데도. 아까 프록시 단일 요청 실측(1.18초)과
모순돼 보여서, "프록시 문제가 아니라 호출 방식 문제일 것"이라는 사용자 가설로
재진단.

- **진단(구현 전 먼저 확인)**: `_configured_law_names()`를 실제로 호출해 세어보니
  `data/tax_subjects.yml` 6개 세목의 `laws:`를 합치면 18개 법령명 → 검색(루트
  단위) 7회 + 상세조회(법령마다 1회) 18회 = **총 25회 요청**, 요청 사이 무조건
  1초씩 자는 코드 때문에 **sleep만 24초**. 25회 × 프록시 왕복(~1.2~1.5초) + 24초
  ≈ 54~62초로 계산 자체가 60초 예산에 아슬아슬하게(또는 확실히) 안 맞음 — 프록시가
  안 된 게 아니라 **소스 하나가 25배 많은 요청을 60초 안에 욱여넣으려던 것**이
  진짜 원인이었다(다른 4개는 요청이 1~2회뿐이라 여유 있었음).
  - 사용자 조사: 검색 응답(`lawSearch.do`)에 `<시행일자>`가 이미 들어있는데
    `search_law()`가 이 필드를 안 뽑고 버려서, 매 법령마다 별도 상세조회
    (`law_detail()`, lawService.do)를 다시 부르는 게 낭비였다는 걸 앞서 프록시
    실측 curl 결과에서 이미 확인해뒀던 상태.
- **수정**(사용자 지시 — "상한을 늘리기보다 요청을 줄이는 쪽으로"):
  - `search_law()`가 `<시행일자>`도 응답에서 바로 추출(`법령ID`/`공포일자` 등과
    같은 자리에 `시행일자` 필드 추가) — 이제 시행일자엔 상세조회가 필요 없다.
  - `law_detail()`(개정이유 `제개정이유내용`용, 검색 응답엔 없는 유일한 필드)은
    **본법(검색 루트와 이름이 같은 항목)에만** 호출 — 시행령/시행규칙은 생략.
    사용자 판단: "실무적으로 값어치 있는 개정이유(예: 법인세법 시행규칙)는
    본법 개정에 딸려오는 경우가 많아 본법 개정이유에 이미 포함돼 있다" — 손실
    감수.
  - 결과: 요청 25회→**14회**(검색 7 + 상세조회 7), sleep 24초→**13초**.
  - **실측 검증**(로컬, 이번엔 law.go.kr DNS가 살아있어 실제 네트워크로 확인
    가능했음 — 지난 세션엔 이 샌드박스에서 DNS 실패로 못 했었음): 실제
    `law_api.fetch()` 실행 → HTTP 요청 정확히 14회, sleep 정확히 13회, 총
    13.5초 소요(계산과 정확히 일치). 18개 항목 전부 `effective_date` 정상
    추출 확인(시행령/시행규칙 포함 — 데이터 손실 없음), `revision_reason`은
    설계대로 본법 7건에만 존재(법인세법/부가가치세법/소득세법/국제조세조정에
    관한 법률/조세특례제한법/지방세법/지방세특례제한법), 시행령/시행규칙
    11건은 `None`으로 확인.
  - `tests/test_law_api.py` 신규 5개(검색 응답 시행일자 추출, 상세조회가
    본법에만 호출되는지, sleep 횟수, 상세조회 실패 시 검색 응답 값으로
    폴백, 검색 실패 시 그 루트만 건너뛰는지) — law_api.py도 기존엔 단위테스트가
    없었어서 이번에 처음 커버. 전체 508→513개 통과.
- **미검증**: GitHub Actions 실측 — push 후 재실행해서 law_api가 60초 안에
  끝나고 성공 9개(kasb/fss/moef/nts/fsc/policy_briefing/law_api/google_news/
  naver_news)가 되는지 확인 필요.
...
사용자가 2026-09-02 세션 마지막에 추가한 항목:
- **한 달 사용 후 재검토 (2026-10월)** — 실사용 한 달 경과 시점에 전반적 재점검: 필터 정확도(`docs/EXCLUDED_LOG.md` 누적분 훑어서 과다/과소 필터링 여부), 신규 항목 메일 알림 실효성(실제로 유용했는지), 요약 캐시 최신성(새로 쌓인 후보 밀림 여부), "현행 기준" 탭 데이터 신선도 등.
- **정기 요약 작업 절차 문서화** — 아래 "정기 요약 작업 절차" 섹션 참고(이번에 처음 명문화 — 기존엔 "주 1~2회 배치 처리"라는 방침만 있고 구체 절차는 코드에만 암묵적으로 있었음).

사용자가 2026-09-01 세션 마지막에 정리한 "남은 일" 중 남은 것:
- **요약은 주 1~2회 몰아서 처리** — 매일 크롤링에서 새 후보가 계속 쌓이므로, 그때그때가 아니라 주기적으로 배치 처리하는 운영 방식으로 확정(아래 "정기 요약 작업 절차" 참고).
- **시행완료 "더보기" 버튼 미표시(우선순위 낮음)** — 원인 조사(console.log 계측)까지 했으나 우선순위 낮음으로 확정, 보류 중(2026-09-02 세션에서도 재확인 — 여전히 낮은 우선순위로 보류).
...
자동/정기 실행 아님 — 사용자가 "요약해줘"로 명시 요청할 때만 아래 흐름을 실행한다.

1. **후보 확인**: `python -m sources.summary_candidates` 실행(`site/data.json`이 최신이어야 함, 없으면 먼저 `python -m sources.main`). `data/summary_cache.json`에 이미 있는 id·상설자료(`is_static`)·논의자료/해외기준(`doc_type`)·`SUMMARIZE_CONFIG.min_tier` 초과 뉴스는 자동 제외되고, 최대 `SUMMARIZE_CONFIG.max_batch`개까지만 나온다.
2. **뉴스(L3) 기사는 대상에서 제외**(확정 방침) — 공식/L1/L2 문서만 요약. `find_candidates()`가 tier 기준으로 이미 대부분 걸러주지만, 애매하면 사람이 한 번 더 확인.
3. **Claude Code가 각 후보의 원문 URL을 직접 읽고** summary 최대 2줄 + impact 1줄(팜한농 제조업·연결 실체 관점)을 작성. 팜한농과 무관하면 impact는 `null`로 두고 억지로 만들지 않는다(summary는 계속 채움).
4. **`sources._summary_cache.write_entry(id, summary=[...], impact=...)`로 저장** — `model="claude-code-manual"`과 `generated_at`이 자동으로 채워진다. 기존 id로 다시 호출하면 덮어쓴다.
5. 다음 `python -m sources.main` 실행 시 `_summarize.summarize()`가 캐시를 우선 조회해 `ai_generated=true`로 반영 → 카드에 "(AI 생성)" 라벨(보라색 강조).
6. `data/summary_cache.json`은 git 추적 대상 — **커밋해서 push**해야 다음 크롤링(로컬이든 GitHub Actions든)에 실제로 반영된다.
